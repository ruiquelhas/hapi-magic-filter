...
internals.registerLifeCycleHandlers = function (stream, reference, callback) {
  var magic;
  ...
  stream.on('end', function () {
    if (!magic) {
      return callback(new Error('Unable to read the magic number.'));
    }
...
internals.validateStream = function (stream, reference, callback) {
  internals.registerLifeCycleHandlers(stream, reference, callback);
};

internals.validateFile = function (file, reference, callback) {
  internals.registerLifeCycleHandlers(fs.createReadStream(file.path), reference, callback);
};
